...
import 'package:life_line_ngo/providers/ngo_chat_screen_provider.dart';
...
class _NgoChatScreenState extends ConsumerState<NgoChatScreen> {
  ...
  Future<void> _initializeChat() async {
    ...
    try {
      ...
      // Victim Firebase (project-life-line)
      try {
        ngoApp = Firebase.app('project-life-line');
      } catch (_) {
      ...
      victimFirestore = FirebaseFirestore.instanceFor(app: ngoApp);
      if (!mounted) return;
      currentNgoId = ref.read(ngoDasboardProvider).ngoDocId;
      ...
      final chatId = _generateChatId(currentNgoId!, widget.victimId);
      if (mounted) {
  ...
  Future<void> _sendMessage() async {
    try {
      ...
      final userId = ref.read(ngoDasboardProvider).ngoDocId;
      if (!mounted) return;
